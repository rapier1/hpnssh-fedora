...
Source7: hpnsshd.socket
Source8: hpnsshd.service
Source9: hpnsshd-keygen@.service
Source10: hpnsshd-keygen
Source11: hpnsshd-keygen.target
Source12: hpnssh-agent.service
Source13: hpnsshd.sysuser
Source14: hpnssh-keys.sysuser
...
Requires: hpnssh%{?_isa} = %{version}-%{release}
...
%description clients
HPNSSH is Pittsburgh Supercomputing Center's version of OpenSSH with
additional functionality and performance. OpenSSH is a free version of
SSH (Secure SHell), a program for logging into and executing commands
on a remote machine. This package includes the clients necessary to
make encrypted connections to SSH servers. This package includes
the HPNSSH extensions to OpenSSH. 
...
additional functionality and performance. OpenSSH is a free version of
SSH (Secure SHell), a program for logging into and executing commands
on a remote machine. This package contains the secure shell daemon
(hpnsshd). The hpnsshd daemon allows SSH clients to securely connect
to your SSH server. This package includes the HPNSSH extension to OpenSSH. 
...
%description askpass
HPNSSH is Pittsburgh Supercomputing Center's version of OpenSSH with
additional functionality and performance. OpenSSH is a free version of SSH
(Secure SHell), a program for logging into and executing commands on
a remote machine. This package contains an X11 passphrase dialog for OpenSSH.
...
%configure \
	...
	--with-privsep-path=%{_datadir}/empty.hpnsshd \
	--disable-strip \
...
%if ! %{no_gnome_askpass}
  ...
  %if %{gtk3}
        ...
        mv gnome-ssh-askpass3 gnome-hpnssh-askpass
  %endif
  %if %{gtk2}
        CFLAGS="$CFLAGS %{?__global_ldflags}" \
           make %{?_smp_mflags} gnome-ssh-askpass2
        mv gnome-ssh-askpass2 gnome-hpnssh-askpass
  %endif
  %if !%{gtk2} && !%{gtk3}	
	CFLAGS="$CFLAGS %{?__global_ldflags}" \
	    make %{?_smp_mflags} gnome-ssh-askpass1
	mv gnome-ssh-askpass1 hpngnome-hpnssh-askpass
  %endif
  popd
%endif
...
mkdir -p -m755 $RPM_BUILD_ROOT%{_sysconfdir}/hpnssh/ssh_config.d
mkdir -p -m755 $RPM_BUILD_ROOT%{_sysconfdir}/hpnssh/sshd_config.d
mkdir -p -m755 $RPM_BUILD_ROOT%{_libexecdir}/hpnssh
mkdir -p -m755 $RPM_BUILD_ROOT%{_datadir}/empty.hpnsshd
mkdir -p -m755 $RPM_BUILD_ROOT/usr/lib/systemd/system
mkdir -p -m755 $RPM_BUILD_ROOT/etc/profile.d
%make_install

install -d $RPM_BUILD_ROOT/etc/pam.d/
install -d $RPM_BUILD_ROOT/etc/sysconfig/
install -d $RPM_BUILD_ROOT%{_libexecdir}/hpnssh
install -m644 %{SOURCE2} $RPM_BUILD_ROOT/etc/pam.d/hpnsshd
install -m644 %{SOURCE4} $RPM_BUILD_ROOT/etc/pam.d/hpnssh-keycat
...
%if ! %{no_gnome_askpass}
ln -s gnome-hpnssh-askpass $RPM_BUILD_ROOT%{_libexecdir}/hpnssh/hpnssh-askpass
install -m 755 -d $RPM_BUILD_ROOT%{_sysconfdir}/profile.d/
install -m 755 contrib/redhat/gnome-ssh-askpass.csh $RPM_BUILD_ROOT%{_sysconfdir}/profile.d/gnome-hpnssh-askpass.csh
install -m 755 contrib/redhat/gnome-ssh-askpass.sh $RPM_BUILD_ROOT%{_sysconfdir}/profile.d/gnome-hpnssh-askpass.sh
%endif

%if %{no_gnome_askpass}
rm -f $RPM_BUILD_ROOT/etc/profile.d/gnome-hpnssh-askpass.*
%endif

perl -pi -e "s|$RPM_BUILD_ROOT||g" $RPM_BUILD_ROOT%{_mandir}/man*/*

%pre
# create the ssh_keys group if necessary
# this is the same name as used in the openssh package
# as the keys will be shared between them in many cases
...
%post server
#generate new server keys if necessary. 
echo "Generate new host keys if necessary."
/usr/libexec/hpnssh/hpnsshd-keygen rsa
/usr/libexec/hpnssh/hpnsshd-keygen dsa
/usr/libexec/hpnssh/hpnsshd-keygen ecdsa
/usr/libexec/hpnssh/hpnsshd-keygen ed25519

#update selinux with the hpnssh entrypoints
if [ -e /usr/sbin/semanage ]
    then
       echo "Updating SELinux for HPNSSH. Please Wait."
       # I have no idea if all of these are necessary but at least one of them is
       semanage fcontext -a -f f -t sshd_key_t  /etc/hpnssh/ssh_host_dsa_key 2> /dev/null
       echo "Completed 1 of 12"
       semanage fcontext -a -f f -t sshd_key_t  /etc/hpnssh/ssh_host_rsa_key 2> /dev/null
       echo "Completed 2 of 12"
       semanage fcontext -a -f f -t sshd_key_t  /etc/hpnssh/ssh_host_ecdsa_key 2> /dev/null
       echo "Completed 3 of 12"
       semanage fcontext -a -f f -t sshd_key_t  /etc/hpnssh/ssh_host_ed25519_key 2> /dev/null
       echo "Completed 4 of 12"
       semanage fcontext -a -f f -t sshd_key_t  /etc/hpnssh/ssh_host_dsa_key.pub 2> /dev/null
       echo "Completed 5 of 12"
       semanage fcontext -a -f f -t sshd_key_t  /etc/hpnssh/ssh_host_rsa_key.pub 2> /dev/null
       echo "Completed 6 of 12"
       semanage fcontext -a -f f -t sshd_key_t  /etc/hpnssh/ssh_host_ecdsa_key.pub 2> /dev/null
       echo "Completed 7 of 12"
       semanage fcontext -a -f f -t sshd_key_t  /etc/hpnssh/ssh_host_ed25519_key.pub 2> /dev/null
       echo "Completed 8 of 12"
       semanage fcontext -a -f f -t sshd_exec_t /usr/sbin/hpnsshd 2> /dev/null
       echo "Completed 9 of 12"
       semanage fcontext -a -f f -t sshd_keygen_exec_t /usr/libexec/hpnssh/hpnsshd-keygen 2> /dev/null
       echo "Completed 10 of 12"
       semanage fcontext -a -f f -t bin_t /usr/libexec/hpnssh/hpnsftp-server 2> /dev/null
       echo "Completed 11 of 12"
       semanage fcontext -a -f f -t etc_t /etc/pam.d/hpnsshd 2> /dev/null
       echo "Completed 12 of 12"
       echo "Setting port in SELinux to allow 2222"
       semanage port -a -t ssh_port_t -p tcp 2222 2> /dev/null
       # once you change the context you need to apply it
...
%systemd_post hpnsshd.service hpnsshd.socket
...
test -f %{sysconfig_anaconda} && \
  test ! -f /etc/hpnssh/sshd_config.d/01-permitrootlogin.conf && \
  grep -q '^PERMITROOTLOGIN="-oPermitRootLogin=yes"' %{sysconfig_anaconda} && \
  grep -q '^Include /etc/hpnssh/sshd_config.d/\*.conf' /etc/hpnssh/sshd_config && \
  echo "PermitRootLogin yes" >> /etc/hpnssh/sshd_config.d/25-permitrootlogin.conf && \
  rm %{sysconfig_anaconda} || :

%preun server
%systemd_preun hpnsshd.service hpnsshd.socket

%postun server
%systemd_postun_with_restart hpnsshd.service

%post clients
if [ -e /usr/sbin/semanage ]
    then
       echo "Updating SELinux for HPNSSH. Please Wait."
       semanage fcontext -a -f f -t ssh_exec_t /usr/bin/hpnssh 2> /dev/null
       echo "Completed 1 of 3"
       semanage fcontext -a -f f -t ssh_agent_exec_t /usr/bin/hpnssh-agent 2> /dev/null
       echo "Completed 2 of 3"
       semanage fcontext -a -f f -t ssh_keygen_exec_t /usr/bin/hpnssh-keygen 2> /dev/null
       echo "Completed 3 of 3"
       # once you change the context you need to apply it
       restorecon /usr/bin/hpnssh
       restorecon /usr/bin/hpnssh-agent
       restorecon /usr/bin/hpnssh-keygen
fi
%systemd_user_post hpnssh-agent.service

%preun clients
%systemd_user_preun hpnssh-agent.service

%files
%license LICENCE
%doc HPN-README CREDITS OVERVIEW PROTOCOL* README README.platform README.privsep README.tun README.dns TODO
%dir %{_sysconfdir}/hpnssh
%config(noreplace) %{_sysconfdir}/hpnssh/moduli
%{_bindir}/hpnssh-keygen
%{_mandir}/man1/hpnssh-keygen.1*
%dir %{_libexecdir}/hpnssh
%attr(2555,root,ssh_keys) %{_libexecdir}/hpnssh/hpnssh-keysign
%{_mandir}/man8/hpnssh-keysign.8*

%files clients
%{_bindir}/hpnssh
%{_mandir}/man1/hpnssh.1*
%{_bindir}/hpnscp
%{_mandir}/man1/hpnscp.1*
%config(noreplace) %{_sysconfdir}/hpnssh/ssh_config
%dir %{_sysconfdir}/hpnssh/ssh_config.d/
%config(noreplace) %{_sysconfdir}/hpnssh/ssh_config.d/50-redhat.conf
%{_mandir}/man5/hpnssh_config.5*
%{_bindir}/hpnssh-agent
%{_bindir}/hpnssh-add
%{_bindir}/hpnssh-keyscan
%{_bindir}/hpnsftp
%{_bindir}/hpnssh-copy-id
%{_libexecdir}/hpnssh/hpnssh-pkcs11-helper
%{_libexecdir}/hpnssh/hpnssh-sk-helper
%{_mandir}/man1/hpnssh-agent.1*
%{_mandir}/man1/hpnssh-add.1*
%{_mandir}/man1/hpnssh-keyscan.1*
%{_mandir}/man1/hpnsftp.1*
%{_mandir}/man1/hpnssh-copy-id.1*
%{_mandir}/man8/hpnssh-pkcs11-helper.8*
%{_mandir}/man8/hpnssh-sk-helper.8*
%{_userunitdir}/hpnssh-agent.service

%files server
%dir %attr(0711,root,root) %{_datadir}/empty.hpnsshd
%{_sbindir}/hpnsshd
%{_libexecdir}/hpnssh/hpnsftp-server
%{_libexecdir}/hpnssh/hpnsshd-keygen
%{_mandir}/man5/hpnsshd_config.5*
%{_mandir}/man5/hpnmoduli.5*
%{_mandir}/man8/hpnsshd.8*
%{_mandir}/man8/hpnsftp-server.8*
%attr(0600,root,root) %config(noreplace) %{_sysconfdir}/hpnssh/sshd_config
%dir %attr(0700,root,root) %{_sysconfdir}/hpnssh/sshd_config.d/
%attr(0600,root,root) %config(noreplace) %{_sysconfdir}/hpnssh/sshd_config.d/50-redhat.conf
%config(noreplace) /etc/pam.d/hpnsshd
%attr(0640,root,root) %config(noreplace) /etc/sysconfig/hpnsshd
%{_unitdir}/hpnsshd.service
%{_unitdir}/hpnsshd@.service
%{_unitdir}/hpnsshd.socket
%{_unitdir}/hpnsshd-keygen@.service
%{_unitdir}/hpnsshd-keygen.target

%files keycat
%doc HOWTO.ssh-keycat
%{_libexecdir}/hpnssh/hpnssh-keycat
%config(noreplace) /etc/pam.d/hpnssh-keycat

%if ! %{no_gnome_askpass}
%files askpass
%{_sysconfdir}/profile.d/gnome-hpnssh-askpass.*
%{_libexecdir}/hpnssh/gnome-hpnssh-askpass
%{_libexecdir}/hpnssh/hpnssh-askpass
%endif

%changelog
* Tue Mar 15 2022 Chris Rapier <rapier@psc.edu> - 8.8.p1_hpn16v1-3
- Incorporating changes from review
- Change user creation method to systemd-rpm-macros method
- Remove superfluous attrs
- Change gpg verification to gpgverify macro
- Update dependencies in Requires: for sub packages
- Moved libexec/openssh to libexec/hpnssh
- Removed pam_ssh_agent sub package.
- Allow building askpass against gtk3

* Thu Feb 10 2022 Chris Rapier <rapier@psc.edu> - 8.8p1_hpn16v1-2
- Start with hpnssh instead of openssh tarball
- Fixed issue with gsskex throwing an error
- Updated to allow port 2222 in selinux
- All patches are rebased against hpnssh as source

* Tue Feb 08 2022 Chris Rapier <rapier@psc.edu> - 8.8p1-1
- Updated to 8.8
- Refactored binary names

* Thu Sep 02 2021 Chris Rapier <rapier@psc.edu> - 8.6p1-4
- Integrating HPN extension
